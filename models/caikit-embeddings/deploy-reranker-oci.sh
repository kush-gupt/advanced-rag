#!/bin/bash
# Deploy MS-MARCO MiniLM-L12-v2 Reranker using OCI Modelcar (no S3 required)
#
# Prerequisites:
...
#
# Usage:
#   ./deploy-reranker-oci.sh [namespace] [registry-owner]
#
# Example:
#   ./deploy-reranker-oci.sh caikit-embeddings myorg
...
echo "Deploying MS-MARCO Reranker (OCI) to namespace: $NAMESPACE"

if [ "$REGISTRY_OWNER" = "REGISTRY_OWNER" ]; then
    echo "Warning: Using placeholder REGISTRY_OWNER."
    echo "Usage: ./deploy-reranker-oci.sh [namespace] [registry-owner]"
    echo "Example: ./deploy-reranker-oci.sh caikit-embeddings myorg"
    echo ""
fi
...
if [ "$REGISTRY_OWNER" != "REGISTRY_OWNER" ]; then
    ...
    sed "s/REGISTRY_OWNER/$REGISTRY_OWNER/g" \
        "$SCRIPT_DIR/manifests/reranker-oci/inference-service.yaml" | \
        oc apply -n "$NAMESPACE" -f -
else
    oc apply -f "$SCRIPT_DIR/manifests/reranker-oci/inference-service.yaml" -n "$NAMESPACE"
fi
...
oc apply -f "$SCRIPT_DIR/manifests/reranker-oci/service.yaml" -n "$NAMESPACE"
oc apply -f "$SCRIPT_DIR/manifests/reranker-oci/route.yaml" -n "$NAMESPACE"

# Get external endpoint
ROUTE_HOST=$(oc get route ms-marco-reranker -n "$NAMESPACE" -o jsonpath='{.spec.host}')
ENDPOINT="https://$ROUTE_HOST"
echo ""
echo "MS-MARCO Reranker (OCI) deployed successfully!"
